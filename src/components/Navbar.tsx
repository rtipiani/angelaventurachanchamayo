  	...
  	return (
    	<nav className="bg-white py-4 shadow-md relative z-50">
      		<div className="container mx-auto flex justify-between items-center px-4 lg:px-6">
				...
				>
            		{isMenuOpen ? <X className="w-8 h-8 focus:outline-none" /> : <Menu className="w-8 h-8 focus:outline-none" />}
        		</button>
      		</div>
      		...
      		<div className={`fixed top-0 right-0 h-full w-[80%] max-w-sm bg-white shadow-2xl z-50 transform transition-all duration-500 ease-in-out lg:hidden ${ isMenuOpen ? "translate-x-0 opacity-100 scale-100" : "translate-x-full opacity-0 scale-95" }`}>
        		<div className="flex flex-col h-full pt-6 pb-6 px-6 overflow-y-auto">
          			...
          			<ul className="flex flex-col space-y-4 text-lg font-medium text-orange-900">
            			{menu.map((item, i) => item.dropdown ? (
                			<li key={i} className="border-b border-orange-100 pb-2">
                  				<button type="button" onClick={() => setMobileDropdownOpenIndex(mobileDropdownOpenIndex === i ? null : i)} className="w-full flex justify-between items-center py-2 text-orange-800 hover:text-red-600 transition-colors focus:outline-none">
                    				{item.label}
          			...
          			<div className="mt-auto pt-8">
            			<a href="https://wa.link/okkl9y" className="block w-full text-center bg-red-600 text-white font-bold py-3 rounded-lg shadow-lg hover:bg-red-700 transition-transform transform hover:scale-105" onClick={() => setIsMenuOpen(false)} target="_blank" rel="noopener noreferrer">
              				Reserva Ahora
            			</a>
          			</div>
        		</div>
